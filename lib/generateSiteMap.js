...
const fetchDynamicPostSlugs = async (postType, label) => {
  ...
  const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
      <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
          ${pages
            ...
            .join('')}
            ${await fetchDynamicPostSlugs('articles', 'article')}
            ${await fetchDynamicPostSlugs('categories', 'category')}
            ${await fetchDynamicPostSlugs('videos', 'video')}
            ${await fetchDynamicPostSlugs('products', 'product')}
      </urlset>
  `
